...
function serialData(data, port) {
	...
	if (data.indexOf('<') == 0) {
		...
		emitToPortSockets(port, 'machineStatus', {'status':t[0], 'workerID': pythonWorkerID, 'chad': chadStatus, 'mpos':[t[2], t[3], t[4]], 'wpos':[t[6], t[7], t[8]]});
...
var pythonWorkerID = -1;
var chadStatus = false;
var workerTimeline = []; // timeline of python worker actions
...
io.sockets.on('connection', function (socket) {
	...
	socket.emit('initialBundle', workerTimeline);
	
	// pairing handshake
	socket.on('handShake', function (data, callback) {
		// clientType: 0 = browser, 1 = pythonWorker
		if (data.clientType = 1 && pythonWorkerID == -1) {
			// set pythonWorkerID as this socket's ID
			pythonWorkerID = socket.id;
			// set socket's serial port to 0
			currentSocketPort[socket.id] = 0;
			sp[0].sockets.push(socket);
			// send callback
			callback('Worker');
			console.log('Worker connected with ID '+ socket.id);
		}else{
	...
	socket.on('start', function (data) {
		if (pythonWorkerID != -1) {
			socket.to(pythonWorkerID).emit('start', 'Boi its time to start');
			chadStatus = true;
		} else {
			socket.emit('serverError', 'Python worker is not online');
		}
	});
	
	socket.on('end', function (data) {
		if (pythonWorkerID != -1) {
			socket.to(pythonWorkerID).emit('end');
			chadStatus = false;
		} else {
			socket.emit('serverError', 'Python worker is not online');
		}
	...
	socket.on('done', function (data) {
		workerTimeline = []; // empty workerTimeline
		emitToPortSockets(0,'done', data); // assume port 0
	});
	
	socket.on('bundle', function (data) {
		if (workerTimeline[data.loop] == null) {
			workerTimeline[data.loop] = [];
		}
		//If is image processed message, buffer image and send
		if(data.event == 0) {
            var itemConversion = {
                1 : "BatteryRect.jpg",
                2 : "CircuitboardRect.jpg",
                3 : "SounddriverRect.jpg",
                4 : "MotorRect.jpg"
            };
            fs.readFile(__dirname + '/images/' + itemConversion[data.type], function(err, buf){
                data.image = buf.toString('base64');
            });
        }
		emitToPortSockets(0,'bundleUpdate', data[workerTimeline[data.loop].push(data)]); // assume port 0
	});
	/**
	socket.on('chadMove', function (data) {
		// split newlines
		var nl = data.line.split("\n");
		// add to queue, assume serial port 1 for 2d movement
		sp[1].q = sp[1].q.concat(nl);
		// add to qCurrentMax
		sp[1].qCurrentMax += nl.length;
		if (sp[1].q.length == nl.length) {
	...
	socket.on('chadMoveZ', function (data) {
		//split newlines and add to serial port 0's queue
		var n1 = data.lin.split("\n");
		sp[0].q = sp[0].q.concat(n1);
		sp[0].qCurrentMax += n1.length;
		if (sp[0].q.length = n1.length) {
			//there was no previous q so write a line
			sendFirstQ(0);
		}
	});
**/
    socket.on('chadMove', function (data) {
        ...
        var nl = data.line.split("\n");
        // add to queue, assume serial port 1 for 2d movement
        sp[0].q = sp[currentSocketPort[socket.id]].q.concat(nl);
        // add to qCurrentMax
	...
	socket.on('disconnect', function() {
		...
		// remove from worker if is worker
		if (pythonWorkerID == socket.id) {
			pythonWorkerID = -1;
			chadStatus = false;
	...
	socket.on('currentPort', function () {
		
		socket.emit('theCurrentPort', currentSocketPort[socket.id]);
